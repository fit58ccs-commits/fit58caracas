"use client";
import { LayoutDashboard, Package, ClipboardList, TrendingUp, Palette, Image, Bell, Search, Menu, User, X } from "lucide-react";
...
export function AdminShell({ section, onSection, pendingOrders, search, onSearch, children }: ShellProps) {
  ...
  const Sidebar = ({ overlay = false }: { overlay?: boolean }) => (
    <aside className={`${overlay ? "relative z-[1]" : ""} flex flex-col shrink-0`}
      ...
      <div className="px-5 py-6 border-b border-neutral-100">
        <div className="text-[8px] font-black text-neutral-400 tracking-[3px] mb-1.5">ADMIN PANEL</div>
        <div className="text-base font-black text-black tracking-tight">DÉLICE GOURMET</div>
      </div>
        ...
        {NAV.map(n => {
          ...
          return (
            <div key={n.id} className="px-0 py-0.5">
              <button
                onClick={() => { onSection(n.id); setMobileOpen(false); }}
                ...
                {n.id === "orders" && pendingOrders > 0 && (
                  <span className="ml-auto bg-red-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-lg shadow-[0_2px_6px_rgba(229,62,62,0.4)]">
                    {pendingOrders}
                  </span>
                )}
              </button>
      ...
      <div className="px-5 py-4 border-t border-neutral-100">
        <div className="flex justify-between text-[10px] text-neutral-400 mb-2 font-semibold uppercase tracking-wide">
          <span>Estado</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-full bg-green-500 inline-block" style={{ animation: "badgePulse 2s infinite", boxShadow: "0 0 6px rgba(34,200,122,0.5)" }} />
          <span className="text-[10px] text-neutral-400">LocalStorage activo</span>
        </div>
      </div>
    </aside>
  );
    ...
    <div className="flex min-h-screen" style={{ background: "linear-gradient(135deg,#f0f2f5 0%,#e8eaed 100%)" }}>
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-[200] flex animate-overlay-in">
          ...
          <div className="animate-drawer-in flex">
            <Sidebar overlay />
          </div>
          <button onClick={() => setMobileOpen(false)} className="absolute top-4 left-[230px] w-9 h-9 bg-white/80 rounded-full flex items-center justify-center border border-neutral-200/80 z-10 cursor-pointer">
            <X size={16} />
          </button>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="glass border-b border-neutral-200/60 px-7 h-14 flex items-center gap-4 sticky top-0 z-[50] rounded-none">
          ...
          <button className="fluent-hover relative w-9 h-9 border border-neutral-200/80 bg-white/65 backdrop-blur-sm flex items-center justify-center rounded-xl cursor-pointer">
            ...
            {pendingOrders > 0 && (
              <span className="absolute top-2 right-2 w-1.5 h-1.5 rounded-full bg-red-500" style={{ boxShadow: "0 0 6px rgba(229,62,62,0.6)" }} />
            )}
          </button>
