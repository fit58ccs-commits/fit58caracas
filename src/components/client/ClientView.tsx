...
import { Package, Heart, ShoppingCart, Award, Globe, Truck, Shield, X, ChevronRight } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-white" id="inicio">
      ...
      <div className="bg-[rgba(17,17,17,0.93)] backdrop-blur-sm h-9 overflow-hidden flex items-center">
        <div className="animate-ticker animate-ticker-pause flex gap-20 whitespace-nowrap">
      ...
      <main className="max-w-[1280px] mx-auto px-4 md:px-7 py-8 pb-36 animate-fade-up">
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-1 -mx-1 px-1">
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => setCategory(cat)}
              className="cat-pill px-4 py-1.5 border rounded-full text-[10px] md:text-[11px] font-bold tracking-wide uppercase whitespace-nowrap shrink-0 cursor-pointer"
              style={{
        ...
        <div className="grid gap-3 md:gap-5"
          ...
          // @ts-ignore
          ref={el => { if (el) { el.style.gridTemplateColumns = window.innerWidth >= 1024 ? "repeat(4,1fr)" : window.innerWidth >= 768 ? "repeat(3,1fr)" : "repeat(2,1fr)"; } }}>
          {filtered.map(p => (
      ...
      <nav className="md:hidden glass fixed bottom-0 left-0 right-0 border-t border-neutral-200/60 flex justify-around py-2.5 pb-5 z-[90] rounded-none shadow-[0_-8px_32px_rgba(0,0,0,0.08)]">
        ...
        {/* Favoritos → abre drawer wishlist */}
        <button onClick={() => setWishlistOpen(true)}
          className="flex flex-col items-center gap-0.5 bg-none border-none cursor-pointer min-w-[44px] relative"
          style={{ color: "#aaa" }}>
          <div className="relative">
            <Heart size={20}/>
      ...
      {wishlistOpen && (
        <div className="fixed inset-0 z-[200] flex">
          ...
          <div className="animate-drawer-in glass absolute right-0 top-0 bottom-0 w-full max-w-[420px] flex flex-col shadow-2xl">
                  ...
                  <div key={p.id} className="glass-card rounded-2xl flex items-center gap-3 p-3">
                    ...
                    <div className="flex flex-col gap-1.5 shrink-0">
                      ...
                      <button onClick={() => store.toggleWishlist(p.id)}
                        className="w-8 h-8 rounded-lg border border-red-200/80 text-red-400 flex items-center justify-center cursor-pointer bg-transparent border-none">
            ...
            {/* Footer — agregar todo al carrito */}
            {wishlistProducts.length > 0 && (
              <div className="px-6 py-4 border-t border-white/40 bg-white/70">
                <button
                  onClick={() => {
                    wishlistProducts.forEach(p => store.addToCart(p));
                  ...
                  AGREGAR TODO AL CARRITO <ChevronRight size={13}/>
                </button>
              </div>
            )}
          </div>
